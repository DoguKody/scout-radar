...
def search_artist_channel(youtube, artist_name, max_results=10):
    ...
    Returns a list of unique candidate channel IDs.
        """
    try:
        print(f"🔎 Primary search for channels matching: {artist_name}")
        channel_ids = set()
        results = {}
        ...
        print(f"✅ Found {len(channel_ids)} unique channels by name.")
        ...
        for i, (cid, meta) in enumerate(results.items(), 1):
            ...
            #print(f"{i}. {meta['title']} → {cid} [channel_search: {cs}, video_search: {vs}]")
